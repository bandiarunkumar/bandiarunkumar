...
CONTRAST = 1.15
BRIGHTNESS = 1.0
GAMMA = 1.25          # >1 brightens mids -> face lands in sparser chars
SHARPEN = True
...
HOST = os.environ.get("GH_PROFILE_USER", "bandiarunkumar")
FULL_NAME = "Bandia Arunkumar"

parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{HOST}@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{HOST}@github:~$ whoami <tspan fill="{INK}">{FULL_NAME}</tspan></text>')

# calculate cursor X offset dynamically based on string length (approx. 7.8px per character)
char_count = len(HOST) + 15 + len(FULL_NAME)
cursor_x = PAD + int(char_count * 7.8)
